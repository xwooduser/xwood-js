...
require('../datepicker/jquery.ui.datepicker.js')($);
require('../core/jquery.ui.slider.js')($);
require('../style/datetimepicker.css');
...
$.datepicker._base_clearDate = $.datepicker._clearDate;
$.datepicker._clearDate = function(id) {
	var target = $(id);
	var inst = this._getInst(target[0]);
	tp_inst = this._get(inst, 'timepicker');
	tp_inst._limitMinMaxDateTime(inst, true);
	this._base_selectDate(id, '');
	this._notifyChange(inst);
	this._updateDatepicker(inst);
};
...
$.datepicker._setDate = function(inst, date, noChange) {
	...
	tp_inst = this._get(inst, 'timepicker');
	if (inst.input) {
		inst.input.val(clear ? '' : this._formatDate(inst)+ tp_inst._defaults.separator + tp_inst.formattedTime + tp_inst._defaults.timeSuffix);
	}
};
